...
void saudio_pcm_lib_preallocate_free_for_all(struct snd_pcm *pcm)
{
	...
	for (stream = 0; stream < 2; stream++)
		for (substream = pcm->streams[stream].substream; substream;
			...
			struct snd_saudio *saudio = snd_pcm_chip(pcm);

			if (substream->dma_buffer.addr) {
				iounmap(substream->dma_buffer.area);
				smem_free(saudio->dst, substream->dma_buffer.addr,
					     ...
					     size_t size, size_t max)
{
	...
	for (stream = 0; stream < SAUDIO_STREAM_MAX; stream++) {
		for (substream = pcm->streams[stream].substream; substream;
			...
			u32 addr = smem_alloc(saudio->dst, size);
			if (addr) {
				dmab->dev.type = type;
				dmab->dev.dev = data;
...
MODULE_DESCRIPTION("saudio driver");
