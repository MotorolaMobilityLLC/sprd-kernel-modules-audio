...
EXPORT_SYMBOL(audio_addr_ap2dsp);
...
static int audio_mem_sharkl5_parse_dt(struct device_node *np)
{
	...
	/* audio cp aon iram */
	memnp = of_parse_phandle(np, "sprd,audcp-aon-iram", 0);
	if (memnp) {
		ret = of_address_to_resource(memnp, 0, &res);
		of_node_put(memnp);
		if (ret)
			return ret;
		audio_mem[IRAM_AUDCP_AON].addr = res.start;
		audio_mem[IRAM_AUDCP_AON].size = res.end + 1 - res.start;
		pr_info("iram audcp aon (addr size), (%#x, %#x)\n",
			audio_mem[IRAM_AUDCP_AON].addr,
			audio_mem[IRAM_AUDCP_AON].size);
	} else {
		pr_err("%s, ERR:Must give me the audio cp aon iram!\n", __func__);
		return -ENODEV;
	}
...
static const struct of_device_id audio_mem_match_table[] = {
	...
	{.compatible = "unisoc,audio-mem-whale2",
	 .data = (void *)PLATFORM_WHALE2},
	{.compatible = "unisoc,audio-mem-sharkl5",
	 .data = (void *)PLATFORM_SHARKL5},
	{.compatible = "unisoc,audio-mem-roc1",
	 .data = (void *)PLATFORM_SHARKL5},
	{.compatible = "unisoc,audio-mem-orca",
