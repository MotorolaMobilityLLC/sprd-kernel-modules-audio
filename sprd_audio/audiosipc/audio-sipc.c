...
static int audio_cmd_copy(void *para, size_t n)
{
	...
	/* write cmd para */
	memcpy_toio((void *)(aud_ipc->param_addr_v), para, n);
...
static int audio_cmd_copy_back(void *para, size_t n)
{
	...
	/* write cmd para */
	memcpy_fromio(para, (void *)(aud_ipc->param_addr_v), n);
...
u32 aud_ipc_dump(void *buf, u32 buf_bytes)
{
	...
	memcpy_fromio(buf, (void *)sipc->smsg_base_v, sipc->size_inout);
	bytes += sipc->size_inout;
	unalign_memset((char *)buf + bytes, PAD_VALUE, PAD_BYTES);
	bytes += PAD_BYTES;
	memcpy_fromio((char *)buf + bytes, (void *)sipc->param_addr_v,
		       sipc->param_size);
	...
	for (i = 0; i < MAX_BLOCK_PARAMETERS; i++) {
		if (sipc->block_param[i].size) {
			if ((bytes + sipc->block_param[i].size) < buf_bytes) {
				memcpy_fromio((char *)buf + bytes, (void *)
...
EXPORT_SYMBOL(aud_get_aud_ipc_smsg_addr);
...
EXPORT_SYMBOL(aud_get_aud_ipc_smsg_para_addr);
...
EXPORT_SYMBOL(aud_ipc_ch_close);
...
EXPORT_SYMBOL(aud_send_cmd_no_wait);
...
EXPORT_SYMBOL(aud_send_cmd_no_param);
...
EXPORT_SYMBOL(aud_send_use_noreplychan);
...
EXPORT_SYMBOL(aud_send_cmd_no_wait_param);
...
EXPORT_SYMBOL(aud_send_cmd_result);
			 ...
			 int type, void *buf, size_t n, int32_t timeout)
{
	...
	/* write cmd para */
	memcpy_toio((void *)(unsigned long)block_param->addr_v, buf, n);
	sharemem_info.id = id;
...
EXPORT_SYMBOL(aud_send_block_param);
			 ...
			 int32_t timeout)
{
	...
	memcpy_fromio(buf, (void *)(unsigned long)block_param->addr_v,
		       block_param->size);

	return 0;
}
EXPORT_SYMBOL(aud_recv_block_param);

static void audio_sipc_rx_callback(struct mbox_client *client, void *message)
{
	aud_smsg_rx_handler(client, message);
}
...
static int audio_sipc_probe(struct platform_device *pdev)
{
	...
	aud_ipc->cl.tx_block     = false;
	aud_ipc->cl.knows_txdone = false;
	aud_ipc->cl.tx_tout      = 500;
	aud_ipc->chan = mbox_request_channel(&aud_ipc->cl, 0);

	if (IS_ERR(aud_ipc->chan)) {
		dev_err(&pdev->dev, "mailbox request chan fail !!!");
