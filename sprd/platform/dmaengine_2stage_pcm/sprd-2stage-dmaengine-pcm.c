/*
 * audio/sprd/dai/sprd-dmaengine-pcm.c
 *
...
#endif
extern void vbc_clock_set_force(void);
extern void vbc_clock_clear_force(void);
			       ...
			       u32 buffer_size)
{
	...
	dma_buffer->private_data = dma_buffer;
	dma_buffer->area =
		dma_alloc_wc(substream->pcm->card->dev,
			      ...
			      struct snd_dma_buffer *dma_buffer)
{
	...
	if (dma_buffer->area &&
	    dma_buffer->dev.type == SNDRV_DMA_TYPE_DEV) {
		dma_free_wc(substream->pcm->card->dev,
...
#if (defined(CONFIG_SND_SOC_UNISOC_VBC_R3P0) || defined(CONFIG_SND_SOC_UNISOC_VBC_R3P0_MODULE))
			       ...
			       struct snd_pcm_hw_params *params)
{
	...
	if (!(params->flags & SNDRV_PCM_HW_PARAMS_NO_PERIOD_WAKEUP)) {
		pr_info("sprd_pcm_hw_params1, SPRD_DMA_FLAGS = SPRD_DMA_TRANS_INT\n");
		callback = sprd_pcm_dma_buf_done;
		flag = SPRD_DMA_FLAGS(0,
				      0, SPRD_DMA_FRAG_REQ, SPRD_DMA_TRANS_INT);
	} else {
		pr_info("sprd_pcm_hw_params1, SPRD_DMA_FLAGS = SPRD_DMA_NO_INT\n");
		flag = SPRD_DMA_FLAGS(0,
...
static int aud_init_iram_addr(struct snd_soc_component *platform)
{
	struct platform_pcm_priv *priv_data = NULL;
	u32 offset = 0;
...
static void aud_clear_iram_addr(struct snd_soc_component *platform)
{
	struct platform_pcm_priv *priv_data = NULL;
...
{
	struct snd_soc_component *platform = NULL;
	struct platform_pcm_priv *priv_data = NULL;
	...
	platform = snd_soc_kcontrol_component(kcontrol);
	if (!platform) {
