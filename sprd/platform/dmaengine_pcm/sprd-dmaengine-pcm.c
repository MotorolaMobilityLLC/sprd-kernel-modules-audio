...
static int sprd_pcm_open(struct snd_soc_component *component, struct snd_pcm_substream *substream)
{
	...
	pr_info("%s %s Open %s\n", __func__, sprd_dai_pcm_name(asoc_rtd_to_cpu(srtd, 0)),
			PCM_DIR_NAME(substream->stream));

	if (sprd_is_i2s_or_tdm(asoc_rtd_to_cpu(srtd, 0))) {
		snd_soc_set_runtime_hwparams(substream, &sprd_i2s_pcm_hardware);
		if (asoc_rtd_to_cpu(srtd, 0)->driver->id == I2S_MAGIC_ID) {
			...
			tdmconf = sprd_tdm_dai_to_config(asoc_rtd_to_cpu(srtd, 0));
			if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
	...
	} else if (be_rtd && asoc_rtd_to_cpu(be_rtd, 0)->id == AGCP_IIS0_TX) {
		snd_soc_set_runtime_hwparams(substream, &sprd_i2s_pcm_hardware);
		burst_len = 0xc0 * 4;
		hw_chan = 1;
	} else {
...
static int sprd_pcm_close(struct snd_soc_component *component, struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
			       ...
			       dma_addr_t *dma_buff_phys)
{
	...
	if (be_rtd && asoc_rtd_to_cpu(be_rtd, 0)->id == AGCP_IIS0_TX)
		dma_data = snd_soc_dai_get_dma_data(asoc_rtd_to_cpu(be_rtd, 0), substream);
	else
						 ...
						 int stream)
{
	...
	if (sprd_is_i2s_or_tdm(asoc_rtd_to_cpu(rtd, 0)) ||
		(be_rtd && asoc_rtd_to_cpu(be_rtd, 0)->id == AGCP_IIS0_TX))
...
static int sprd_pcm_new(struct snd_soc_component *component, struct snd_soc_pcm_runtime *rtd)
{
...
static void sprd_pcm_free_dma_buffers(struct snd_soc_component *component, struct snd_pcm *pcm)
{
	sp_asoc_pr_dbg("%s\n", __func__);
}
...
static void sprd_pcm_platform_proc_init(struct snd_soc_component *platform)
{
...
#else /* !CONFIG_PROC_FS */
static inline void
sprd_pcm_platform_proc_init(struct snd_soc_component *platform)
{
}
#endif
