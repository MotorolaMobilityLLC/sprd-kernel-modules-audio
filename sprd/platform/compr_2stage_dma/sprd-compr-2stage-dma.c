...
#define SPRD_DMAENGINE_COMPR_DRV_NAME "sprd-compr-dma"
...
static int sprd_compress_new(struct snd_soc_component *component, struct snd_soc_pcm_runtime *rtd);
			      ...
			      u32 frag_size, u32 frag_cnt)
{
	...
	struct snd_soc_component *platform
		= snd_soc_rtdcom_lookup(rtd, SPRD_DMAENGINE_COMPR_DRV_NAME);
	...
	dma_chn_request = dma_request_chan(
					pdata->dev,
					"compr_dma_stage1");
	...
	cfg_ptr->ll_cfg.wrap_addr = audio_addr_ap2dsp(IRAM_OFFLOAD,
	...
	pr_info("cfg_ptr->ll_cfg.phy_addr:%p,src:%p,dst:%p,wrap_addr:%p,dma_chan:%d\n",
		...
		(void *)(unsigned long)cfg_ptr->ll_cfg.wrap_addr,
					...
					struct snd_compr_stream *cstream)
{
	...
	sprd_compress_new(component, rtd);
	...
	srtd->wake_lock = wakeup_source_create("compr write");
	wakeup_source_add(srtd->wake_lock);
						...
						struct snd_compr_stream *cstream)
{
	...
	if (srtd->stream_state == COMPR_TRIGGERED)
		sprd_platform_compr_trigger(component, cstream, SNDRV_PCM_TRIGGER_STOP);
	...
	wakeup_source_remove(srtd->wake_lock);
	wakeup_source_destroy(srtd->wake_lock);
...
static int sprd_platform_compr_ack(struct snd_soc_component *component,
					struct snd_compr_stream *cstream,
					size_t bytes)
{
	ADEBUG();
...
static int sprd_platform_compr_get_codec_caps(struct snd_soc_component *component,
					struct snd_compr_stream *cstream,
...
static int sprd_platform_compr_set_metadata(struct snd_soc_component *component,
						struct snd_compr_stream *cstream,
...
static void sprd_compress_proc_init(struct snd_soc_component *platform)
{
	struct snd_info_entry *entry;

	sp_asoc_pr_dbg("%s E", __func__);
	if (!snd_card_proc_new(platform->card->snd_card,
		"sprd-compress", &entry))
...
static int sprd_compr_probe(struct snd_soc_component *platform)
{
	...
	snd_soc_component_set_drvdata(platform, pdata);
...
static int sprd_compress_new(struct snd_soc_component *component,
...
static struct snd_compress_ops sprd_platform_compr_ops = {
...
static struct snd_soc_component_driver sprd_platform_compr_drv = {
	.name		= SPRD_DMAENGINE_COMPR_DRV_NAME,
	.probe		= sprd_compr_probe,
	.compress_ops	= &sprd_platform_compr_ops,
	.pcm_construct    = sprd_compress_new,
};
...
static int sprd_platform_compr_probe(struct platform_device *pdev)
{
	...
	ret = snd_soc_register_component(&pdev->dev, &sprd_platform_compr_drv, NULL, 0);
	if (ret) {
