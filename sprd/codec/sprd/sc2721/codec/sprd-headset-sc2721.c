...
extern int vbc_close_fm_dggain(bool mute);
...
static struct device_node *sprd_audio_codec_get_card0_node(void)
{
	...
	const char * const comp[] = {
		"unisoc,vbc-r1p0v3-codec-sc2721",
		"unisoc,vbc-r1p0v3-codec-sc2731",
		"unisoc,vbc-r3p0-codec-sc2731",
	};
...
int sprd_headset_soc_probe(struct snd_soc_component *codec)
{
	...
	hdst->det_all_wakelock = wakeup_source_create("headset_detect_wakelock");
	wakeup_source_add(hdst->det_all_wakelock);
	hdst->btn_wakelock = wakeup_source_create("headset_button_wakelock");
	wakeup_source_add(hdst->btn_wakelock);
...
failed_to_request_irq:
	wakeup_source_remove(hdst->btn_wakelock);
	wakeup_source_destroy(hdst->btn_wakelock);
	wakeup_source_remove(hdst->det_all_wakelock);
	wakeup_source_destroy(hdst->det_all_wakelock);
