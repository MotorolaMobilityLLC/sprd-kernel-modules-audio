/*
 * bsp/modules/kernel5.15/audio/sprd/codec/sprd/sc2721/codec/sprd-codec.c
 *
...
typedef int (*sprd_codec_pga_set) (struct snd_soc_component *codec, int pgaval);
...
typedef int (*sprd_codec_switch_set) (struct snd_soc_component *codec, int on);
...
typedef int (*sprd_codec_mixer_set) (struct snd_soc_component *codec, int on);
...
extern int xtlbuf1_eb_set(void);
extern int xtlbuf1_eb_clr(void);
...
static int sprd_codec_auto_ldo_volt(
				    struct snd_soc_component *codec,
				    void (*set_level)(struct snd_soc_component *, int),
	const struct sprd_codec_ldo_v_map *v_map,
			    ...
			    int pgaval)
{
	...
	val = is_class_d ? ((pgaval << PA_D_G) | (PA_AB_G_0DB << PA_G)) :
		(pgaval << PA_G);
	return snd_soc_component_update_bits(codec, SOC_REG(reg), mask, val);
}
...
static int sprd_codec_pga_hpl_set(struct snd_soc_component *codec, int pgaval)
{
	int reg, val, mask;
...
static int sprd_codec_pga_hpr_set(struct snd_soc_component *codec, int pgaval)
{
	int reg, val, mask;
...
static int sprd_codec_pga_ear_set(struct snd_soc_component *codec, int pgaval)
{
	int reg, val, mask;
...
static int sprd_codec_pga_adcl_set(struct snd_soc_component *codec, int pgaval)
{
	int reg, val, mask;
...
static int sprd_codec_pga_adcr_set(struct snd_soc_component *codec, int pgaval)
{
	int reg, val, mask;
...
static int sprd_codec_pga_daclr_set(struct snd_soc_component *codec, int pgaval)
{
	int reg, val, mask;
...
static int sprd_das_dc_os_en(struct snd_soc_component *codec, int on)
{
	...
	mask = BIT(DAS_OS_EN);
	val = on ? mask : 0;
	snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC3), mask, val);

	return 0;
}

static void sprd_das_dc_os_set(struct snd_soc_component *codec, int offset)
{
	int mask, val;

	mask = DAS_OS_D_MASK << DAS_OS_D;
	val = offset << DAS_OS_D;
	snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC1), mask, val);
}
...
static int sprd_codec_switch_dacs_set(struct snd_soc_component *codec, int on)
{
	int val, mask;

	mask = BIT(DAS_EN);
	val = on ? mask : 0;
	snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC2), mask, val);
...
static int sprd_codec_switch_dacl_set(struct snd_soc_component *codec, int on)
{
	int val, mask;
...
static int sprd_codec_switch_dacr_set(struct snd_soc_component *codec, int on)
{
	int val, mask;
...
/* adc mixer */
static int mainmicadcl_set(struct snd_soc_component *codec, int on)
{
	sp_asoc_pr_dbg("%s %d\n", __func__, on);
	return snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC3), BIT(SMIC1PGAL),
		on << SMIC1PGAL);
}

static int mainmicadcr_set(struct snd_soc_component *codec, int on)
{
...
static int auxmicadcl_set(struct snd_soc_component *codec, int on)
{
...
static int auxmicadcr_set(struct snd_soc_component *codec, int on)
{
	sp_asoc_pr_dbg("%s %d\n", __func__, on);
	return snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC3),
		BIT(SMIC2PGAR), on << SMIC2PGAR);
}

static int hpmicadcl_set(struct snd_soc_component *codec, int on)
{
	sp_asoc_pr_dbg("%s %d\n", __func__, on);
	return snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC3),
...
static int hpmicadcr_set(struct snd_soc_component *codec, int on)
{
	sp_asoc_pr_dbg("%s %d\n", __func__, on);
	return snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC3),
...
/* hp mixer */
static int daclhpl_set(struct snd_soc_component *codec, int on)
{
	sp_asoc_pr_dbg("%s %d\n", __func__, on);
...
static int daclhpr_set(struct snd_soc_component *codec, int on)
{
...
static int dacrhpl_set(struct snd_soc_component *codec, int on)
{
	sp_asoc_pr_dbg("%s %d. DACL can't be connected to HPR.\n",
		       __func__, on);
	return 0;
...
static int dacrhpr_set(struct snd_soc_component *codec, int on)
{
	sp_asoc_pr_dbg("%s %d\n", __func__, on);
	return snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC3),
...
static int adclhpl_set(struct snd_soc_component *codec, int on)
{
	sp_asoc_pr_dbg("%s %d\n", __func__, on);
	return 0;
}

static int adclhpr_set(struct snd_soc_component *codec, int on)
{
...
static int adcrhpl_set(struct snd_soc_component *codec, int on)
{
...
static int adcrhpr_set(struct snd_soc_component *codec, int on)
{
	sp_asoc_pr_dbg("%s %d\n", __func__, on);
	return 0;
}
...
/* spkl mixer */
static int daclspkl_set(struct snd_soc_component *codec, int on)
{
	int ret = 0;
	int enable;
	struct sprd_codec_priv *sprd_codec = snd_soc_component_get_drvdata(codec);
	...
	if (enable) {
		...
		ret = snd_soc_component_update_bits(codec,
...
static int dacrspkl_set(struct snd_soc_component *codec, int on)
{
	return 0;
}

static int adclspkl_set(struct snd_soc_component *codec, int on)
{
	sp_asoc_pr_dbg("%s %d\n", __func__, on);
	return 0;
}

static int adcrspkl_set(struct snd_soc_component *codec, int on)
{
	sp_asoc_pr_dbg("%s %d\n", __func__, on);
	return 0;
}
...
/* spkr mixer */
static int daclspkr_set(struct snd_soc_component *codec, int on)
{
	sp_asoc_pr_dbg("%s %d. No right speaker anymore.\n", __func__, on);
	return 0;
}

static int dacrspkr_set(struct snd_soc_component *codec, int on)
{
	sp_asoc_pr_dbg("%s %d. No right speaker anymore.\n", __func__, on);
	return 0;
}

static int adclspkr_set(struct snd_soc_component *codec, int on)
{
	sp_asoc_pr_dbg("%s %d. No right speaker.\n", __func__, on);
	return 0;
}

static int adcrspkr_set(struct snd_soc_component *codec, int on)
{
	sp_asoc_pr_dbg("%s %d. No right speaker.\n", __func__, on);
	return 0;
}
...
static int daclear_set(struct snd_soc_component *codec, int on)
{
	sp_asoc_pr_dbg("%s %d\n", __func__, on);
	return snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC3), BIT(SDALRCV),
				   on << SDALRCV);
}
...
static inline void sprd_codec_pa_d_en(struct snd_soc_component *codec, int on)
{
	...
	mask = BIT(PA_D_EN);
	val = on ? mask : 0;
	snd_soc_component_update_bits(codec, SOC_REG(ANA_PMU5), mask, val);
}
...
static inline void sprd_codec_pa_emi_rate(struct snd_soc_component *codec, int rate)
{
	...
	snd_soc_component_update_bits(codec, SOC_REG(ANA_PMU4), mask, val);
}

static inline void sprd_codec_pa_ovp_v_sel(struct snd_soc_component *codec,
					   int v_sel)
{
	int mask;
	int val;
	...
	snd_soc_component_update_bits(codec, SOC_REG(ANA_PMU3), mask, val);
}
...
static enum PA_SHORT_T codec_pa_short_check(struct snd_soc_component *codec)
{
	int ret = 0;
	unsigned int val;

	ret = snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC2), BIT(PA_EN), 0);
	if (ret < 0)
		goto CHECK_FAILED;

	/* VBAT short check  */
	ret = snd_soc_component_update_bits(codec, SOC_REG(ANA_PMU5),
	...
	/* GND short check  */
	snd_soc_component_update_bits(codec, SOC_REG(ANA_PMU5),
...
static int sprd_inter_speaker_pa(struct snd_soc_component *codec, int on)
{
	struct sprd_codec_priv *sprd_codec = snd_soc_component_get_drvdata(codec);
...
/*TODO: add dc offset to AudioTestor.*/
static int sprd_dalr_dc_os_en(struct snd_soc_component *codec, int on)
{
	int mask, val;
...
static void sprd_dalr_dc_os_set(struct snd_soc_component *codec, int offset)
{
	int mask, val;
...
static inline int sprd_codec_vcom_ldo_auto(struct snd_soc_component *codec,
					   int init)
{
	struct sprd_codec_priv *sprd_codec = snd_soc_component_get_drvdata(codec);
...
static void sprd_codec_analog_clk_on(struct snd_soc_component *codec, int on)
{
	u32 mask = 0, val = 0;
	...
	snd_soc_component_update_bits(codec, SOC_REG(ANA_CLK0), mask, val);
}

static int sprd_codec_analog_open(struct snd_soc_component *codec)
{
	sp_asoc_pr_dbg("%s\n", __func__);
...
static int sprd_codec_digital_open(struct snd_soc_component *codec)
{
	...
	/*Bug 362021*/
	snd_soc_component_update_bits(codec, SOC_REG(AUD_DAC_SDM_L), 0xFFFF, 0X9999);
...
static void sprd_codec_cfga_clk_enable(struct snd_soc_component *codec, int en)
{
	int msk, val;

	msk = BIT(CLK_AUD_6P5M_EN) | BIT(CLK_AUD_1K_EN) | BIT(CLK_AUD_32K_EN);
	val = en ? msk : 0;
	snd_soc_component_update_bits(codec, SOC_REG(AUD_CFGA_CLK_EN), msk, val);
	msk = BIT(DAC_POST_SOFT_RST) | BIT(DIG_6P5M_SOFT_RST)
		| BIT(AUD_32K_SOFT_RST) | BIT(AUD_1K_SOFT_RST);
	snd_soc_component_update_bits(codec, SOC_REG(AUD_CFGA_SOFT_RST), msk, msk);
	udelay(10);
	snd_soc_component_update_bits(codec, SOC_REG(AUD_CFGA_SOFT_RST), msk, 0);
}
...
static int enable_oxp(struct snd_soc_component *codec)
{
	...
	return snd_soc_component_update_bits(codec, SOC_REG(ANA_PMU3), mask, val);
}

static int adc_low_power_setting(struct snd_soc_component *codec)
{
	int ret = 0;
	...
	ret |= snd_soc_component_update_bits(codec, SOC_REG(ANA_PMU2), mask, val);

	/* Decrease the frequence of ADC clock. */
	mask = AD_CLK_F_MASK << AD_CLK_F;
	val = AD_CLK_F_HALF << AD_CLK_F;
	ret |= snd_soc_component_update_bits(codec, SOC_REG(ANA_CLK0), mask, val);
...
static void sprd_codec_power_enable(struct snd_soc_component *codec)
{
	...
	if (ret < 0)
		pr_err("ERR: %s set VB to normal mode failed!, ret=%d\n", __func__, ret);
...
static void sprd_codec_adc_clock_input_en(struct snd_soc_component *codec, int on)
{
	sp_asoc_pr_dbg("%s, on:%d\n", __func__, on);
			       ...
			       struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL0), msk, msk);
		msk = BIT(DCL_RST);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL0), msk, msk);
		udelay(10);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL0), msk, 0);
		break;
	default:
		WARN_ON(1);
		ret = -EINVAL;
...
static int mixer_set_mixer(struct snd_soc_component *codec, int id, int lr,
...
static inline int mixer_setting(struct snd_soc_component *codec, int start,
...
static inline int mixer_setting_one(struct snd_soc_component *codec, int id,
				    int try_on, int need_set)
{
	int lr = id & 0x1;
...
static inline void codec_print_oxp_stat(struct snd_soc_component *codec)
{
	unsigned int val;
...
static irqreturn_t sprd_codec_ap_irq(int irq, void *dev_id)
{
	...
	snd_soc_component_update_bits(codec, SOC_REG(AUD_CFGA_INT_MODULE_CTRL),
			    val << AUD_A_INT_EN, 0);
...
static int sprd_codec_charge_ext_cap(struct snd_soc_component *codec)
{
	int val;
	int ret = 0;
	int cnt = 20;
	...
	snd_soc_component_update_bits(codec, SOC_REG(ANA_STS2), val, val);
	sprd_codec_wait(2);

	/* Start charging */
	snd_soc_component_update_bits(codec, SOC_REG(ANA_STS2), BIT(DEPOP_CHG_START),
			    BIT(DEPOP_CHG_START));
			  ...
			  struct snd_kcontrol *kcontrol, int event)
{
	...
	return snd_soc_component_update_bits(codec, SOC_REG(ANA_STS2),
			   ...
			   struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		snd_soc_component_update_bits(codec, SOC_REG(ANA_PMU2), mask, val);

		/* Hp driver dummy on */
		val = left ? BIT(HPL_FLOOPEN) : BIT(HPR_FLOOPEN);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC2), mask_drv, val);
		sprd_codec_wait(1);
		/* Hp driver on */
	...
	case SND_SOC_DAPM_PRE_PMD:
		val = left ? BIT(HPL_FLOOPEN) : BIT(HPR_FLOOPEN);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC2), mask_drv, val);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC2), mask_drv, 0);
		break;
	default:
		WARN_ON(1);
		ret = -EINVAL;
			    ...
			    struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC2),
				    mask, BIT(RCV_FLOOPEN));
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC2), mask, 0);
		break;
	default:
		WARN_ON(1);
		ret = -EINVAL;
...
static int adcpgal_set(struct snd_soc_component *codec, int on)
{
	int value = on ? BIT(ADPGAL_EN) : 0;

	return snd_soc_component_update_bits(codec,
		SOC_REG(ANA_CDC0), BIT(ADPGAL_EN), value);
}

static int adcpgar_set(struct snd_soc_component *codec, int on)
{
...
static int adcpgar_byp_set(struct snd_soc_component *codec, int value)
{
	int mask = ADPGAR_BYP_MASK << ADPGAR_BYP;

	return snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC0), mask,
				    value << ADPGAR_BYP);
}
			  ...
			  struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC0), mask, mask);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC0), mask, 0);
		break;
	default:
		WARN_ON(1);
		ret = -EINVAL;
			 ...
			 struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		snd_soc_component_update_bits(codec, SOC_REG(AUD_TOP_CTL), mask, mask);
		ref_cnt[index]++;
		break;
	case SND_SOC_DAPM_POST_PMD:
		if (ref_cnt[index]-- == 1)
			snd_soc_component_update_bits(codec, SOC_REG(AUD_TOP_CTL),
					    mask, 0);
		break;
	default:
...
static int codec_hp_dc_cal(struct snd_soc_component *codec)
{
	int ret = 0;
	unsigned int mask, val;
	...
	snd_soc_component_update_bits(codec, SOC_REG(ANA_STS3), mask, val);

	snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL0), BIT(DPOP_AUTO_RST), 0);
	...
	snd_soc_component_write(codec, SOC_REG(ANA_STS2), mask);
	snd_soc_component_write(codec, SOC_REG(ANA_DCL4), 0xFFFF);
	adie_chip_id = sci_get_ana_chip_id();
	adie_chip_id = (adie_chip_id >> 16) & 0xffff;
	if (adie_chip_id == CHIP_ID_2720)
		snd_soc_component_write(codec, SOC_REG(ANA_DCL5), 0xF000);
	else
		snd_soc_component_write(codec, SOC_REG(ANA_DCL5), 0xFA10);
	snd_soc_component_write(codec, SOC_REG(ANA_DCL6), 0x4CD8);
	snd_soc_component_write(codec, SOC_REG(ANA_DCL7), 0x2E6C);
	snd_soc_component_write(codec, SOC_REG(ANA_STS0), 0xA820);
	sprd_codec_wait(2);

	/* Start calibrating */
	snd_soc_component_update_bits(codec, SOC_REG(ANA_STS2),
...
static void _dacspkl_switch_nolock(struct snd_soc_component *codec, int val)
{
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	if (val) {
		ret = snd_soc_dapm_force_enable_pin(
			&codec->card->dapm, mic_bias_name[id]);
	} else {
...
static int sprd_codec_soc_suspend(struct snd_soc_component *codec)
{
	...
	if (regu && regulator_is_enabled(regu) &&
		...
		ret = snd_soc_component_update_bits(codec, ANA_PMU0,
				    ...
				    struct snd_info_buffer *buffer)
{
	...
	if (pa_en)
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC2), pa_en, pa_en);
...
static int sprd_codec_soc_probe(struct snd_soc_component *codec)
{
	...
	sp_asoc_pr_info("%s\n", __func__);
	sprd_codec_power_regulator_init(sprd_codec, codec->dev);
...
static int sprd_codec_dig_probe(struct platform_device *pdev)
{
	...
	if (soc_type == SOC_TYPE_WHALE2) {
		...
		pr_err("%s, platform_get_irq start\n", __func__);
		val = platform_get_irq(pdev, 0);
		if (val > 0) {
			sprd_codec->dp_irq = val;
			pr_err("Set DP IRQ is %u!\n", val);
		} else {
			pr_err("ERR:Must give me the DP IRQ!\n");
			return -EINVAL;
		}
		pr_err("%s, platform_get_irq end\n", __func__);
...
static int sprd_codec_ana_probe(struct platform_device *pdev)
{
	...
	pr_err("%s, devm_request_threaded_irq end\n", __func__);
