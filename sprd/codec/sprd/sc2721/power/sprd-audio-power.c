/*
 * bsp/modules/kernel5.15/audio/sprd/codec/sprd/sc2721/power/sprd-audio-power.c
 *
		...
		struct regulator_dev *rdev, unsigned int mode)
{
	...
	sp_asoc_pr_info("%s Set Mode 0x%x\n", rdev->desc->name, mode);
...
static int sprd_audio_power_sleep_ctrl(void *data, int en)
{
	...
	sp_asoc_pr_info("sprd_audio_power_sleep_ctrl, id=%d\n", info->desc.id);
	switch (info->desc.id) {
