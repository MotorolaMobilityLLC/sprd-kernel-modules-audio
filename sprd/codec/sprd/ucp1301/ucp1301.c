...
static int ucp1301_spk2_soc_probe(struct snd_soc_component *codec)
{
	struct snd_soc_dapm_context *dapm = snd_soc_component_get_dapm(codec);
	struct ucp1301_t *ucp1301 = snd_soc_component_get_drvdata(codec);
...
static int ucp1301_rcv_soc_probe(struct snd_soc_component *codec)
{
	struct snd_soc_dapm_context *dapm = snd_soc_component_get_dapm(codec);
	struct ucp1301_t *ucp1301 = snd_soc_component_get_drvdata(codec);
...
static struct snd_soc_component_driver soc_codec_dev_ucp1301 = {
...
static struct snd_soc_component_driver soc_codec_dev_ucp1301_spk2 = {
...
static struct snd_soc_component_driver soc_codec_dev_ucp1301_rcv = {
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (strcmp(client->name, ucp1301_type[0]) == 0) {
		ret = snd_soc_register_component(dev, &soc_codec_dev_ucp1301,
					     &ucp1301_dai[0],
					     ARRAY_SIZE(ucp1301_dai));
	} else if (strcmp(client->name, ucp1301_type[1]) == 0) {
		ret = snd_soc_register_component(dev, &soc_codec_dev_ucp1301_spk2,
					     &ucp1301_dai_spk2[0],
					     ARRAY_SIZE(ucp1301_dai_spk2));
	} else if (strcmp(client->name, ucp1301_type[2]) == 0) {
		ret = snd_soc_register_component(dev, &soc_codec_dev_ucp1301_rcv,
					     &ucp1301_dai_rcv[0],
...
static const struct of_device_id extpa_of_match[] = {
	...
	{ .compatible = "unisoc,ucp1301-spk2" },
