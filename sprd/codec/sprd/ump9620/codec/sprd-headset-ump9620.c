...
#define pr_fmt(fmt) pr_sprd_fmt("HDST9620")""fmt
...
{
	return intc_status & FGU_LOW_LIMIT_INT_SHADOW_STATUS;
}
...
int sprd_headset_soc_probe(struct snd_soc_component *codec)
{
	...
	pr_info("%s adie_chip_id 0x%x 0x%x\n", __func__, adie_chip_id >> 16,
		adie_chip_id & 0xffff);
...
static int sprd_headset_parse_dt(struct sprd_headset *hdst)
{
	...
	ret = of_property_read_u32(np, "typec-i2c", &typec_i2c_val);
	if (!ret) {
		hdst->typec_i2c_en = typec_i2c_val;
		pr_info("%s use typec_i2c switch\n", __func__);
		return 0;
	}
...
static int sprd_get_adc_cal_from_efuse(struct platform_device *pdev)
{
	...
	delta[0] = (test[0] & 0xFFF0) >> 4;
	delta[1] = (test[1] & 0xFFF0) >> 4;
	delta[2] = test[2] & 0xFF;
	delta[3] = (test[2] & 0xFF00) >> 8;

	pr_info("%s test[0] 0x%x %d, test[1] 0x%x %d, test[2] 0x%x %d\n",
		__func__, test[0], test[0], test[1], test[1], test[2], test[2]);
	...
	adc_cal_headset.A = delta[0];
	adc_cal_headset.B =  delta[1];
	adc_cal_headset.E1 = delta[2] * 2 + 2400;
	if (delta[3] & 0x80)
		adc_cal_headset.E2 = (((~(delta[3] - 0x80)) & 0x000F) + 1) * 4 + 1500;
	else
