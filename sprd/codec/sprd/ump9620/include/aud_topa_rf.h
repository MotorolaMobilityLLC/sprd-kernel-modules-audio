...
static inline void sprd_codec_audif_et_en(struct snd_soc_component *codec, int on)
{
	int mask = 0;
	int val = 0;
	...
	val = on ? mask:0;
	snd_soc_component_update_bits(codec, SOC_REG(AUD_CFGA_ANA_ET2), mask, val);
}

static inline void sprd_codec_audif_dc_os_set(struct snd_soc_component *codec,
	int offset)
{
	int mask, val;
	...
	snd_soc_component_update_bits(codec, SOC_REG(AUD_CFGA_ANA_ET1), mask, val);
}

static void sprd_codec_audif_clk_enable(struct snd_soc_component *codec, int en)
{
	int msk, val;
