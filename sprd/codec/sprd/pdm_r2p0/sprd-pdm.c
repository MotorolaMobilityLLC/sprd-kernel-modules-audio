...
static int pdm_soc_probe(struct snd_soc_component *codec)
{
...
static struct snd_soc_component_driver soc_codec_dev_pdm = {
	.probe = pdm_soc_probe,
...
static int pdm_probe(struct platform_device *pdev)
{
	...
	ret = snd_soc_register_component(&pdev->dev, &soc_codec_dev_pdm,
				     &pdm_dai[0],
				     ARRAY_SIZE(pdm_dai));
	if (ret)
