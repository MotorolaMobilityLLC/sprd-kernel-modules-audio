/*
 * audio/sprd/codec/sprd/sc2721/sprd-codec.c
 *
...
enum {
	CODEC_PRODUCT_TSMC,
	CODEC_PRODUCT_JINGHE_PHASE1,
	CODEC_PRODUCT_JINGHE_PHASE3,
	CODEC_PRODUCT_MAX,
};
...
static void sprd_codec_pa_boost(struct snd_soc_component *codec, int pa_d_en)
{
	...
	if (pa_d_en) {
		...
		mask = PABST_EN;
		if ((sprd_codec->codec_product_info & 0x60) == 0x20)
		...
		sprd_codec_wait(10);
		while (i++ < 10) {
			state = snd_soc_component_read(codec, SOC_REG(ANA_STS8)) &
				PABST_SOFT_DVLD;
			if (state)
				break;
			sprd_codec_wait(1);
...
static void spk_pa_short_check(struct sprd_codec_priv *sprd_codec)
{
	...
	sprd_codec_wait(1);
	flag = snd_soc_component_read(codec, SOC_REG(ANA_CDC14)) | PA_SHT_FLAG(0xFFFF);
	if (flag) {
	...
	val = snd_soc_component_read(codec, SOC_REG(ANA_STS14)) | IMPD_ADC_DATO(0xFFFF);
			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	if (on) {
		...
		sprd_codec_wait(60);
		while (i++ < 135) {
			state = snd_soc_component_read(codec, SOC_REG(ANA_STS11)) &
					     PA_AB_DPOP_DVLD;
			if (state)
				break;
			sprd_codec_wait(1);
...
static void sprd_codec_sdm_ramp(struct snd_soc_component *codec, bool on)
{
	...
	if (on) {
		...
		val = snd_soc_component_read(codec, SOC_REG(AUD_DAC_SDM_L));
		while (val > 0x20) {
			val -= 0x20;
			snd_soc_component_update_bits(codec, SOC_REG(AUD_DAC_SDM_L),
					0xffff, val);
...
static unsigned int sprd_codec_get_fgu_vbatt(struct snd_soc_component *codec)
{
	...
	vbatt = snd_soc_component_read(codec, SOC_REG(ANA_STS7));
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((sprd_codec->codec_product_info & 0x60) == 0)
		ucontrol->value.integer.value[0] = CODEC_PRODUCT_TSMC;
	else if ((sprd_codec->codec_product_info & 0x60) == 0x20)
		ucontrol->value.integer.value[0] = CODEC_PRODUCT_JINGHE_PHASE1;
	else if ((sprd_codec->codec_product_info & 0x60) == 0x60)
