...
static char *dai_id_aif_name_playback[1] = {
	"AGCP_IIS0_TX",		/* AGCP_IIS0_TX Playback */
};

static char *dai_id_aif_name_capture[1] = {
	NULL,		/* AGCP_IIS0_TX Capture */
};
		...
		int rate, int channel, int bit_width)
{
	...
	struct sprd_dai_data *dai_data = container_of(agcp_i2s, struct sprd_dai_data, agcp_i2s);
	...
	if (!agcp_ahb_gpr)
		return -1;
	ret = pm_runtime_get_sync(dai_data->dev);
	if (ret < 0)
		return ret;
	sprd_dai_reg_update(
	...
	case 32000:
		sprd_dai_reg_update(
				...
				0x5 << 13, 0x7 << 13);
		src_clk = 163840000;
		ret = aud_send_cmd_no_param(AMSG_CH_MISC, SET_AUD_DVFS,
			IIS_CLK_614_4M, 0, 0, 0, -1);
		if (ret < 0)
			pr_err("%s: set DVFS (%d) failed\n", __func__, IIS_CLK_614_4M);
		break;
	case 9600:
	case 12000:
	case 24000:
	case 48000:
		...
		src_clk = 245760000;
		ret = aud_send_cmd_no_param(AMSG_CH_MISC, SET_AUD_DVFS,
			IIS_CLK_245_76M, 0, 0, 0, -1);
		if (ret < 0)
			pr_err("%s: set DVFS (%d) failed\n", __func__, IIS_CLK_245_76M);
		break;
	case 96000:
	case 192000:
		sprd_dai_reg_update(
	...
	pm_runtime_mark_last_busy(dai_data->dev);
	pm_runtime_put_autosuspend(dai_data->dev);
		...
		struct snd_soc_dai *dai)
{
	...
	if (atomic_read(&agcp_i2s->open_cnt) == 0) {
		...
		if (ret < 0)
			return ret;
		ret = aud_send_cmd_no_param(AMSG_CH_MISC, ENABLE_AUD_PLL_BIND,
			0, 0, 0, 0, -1);
		if (ret == 0)
			agcp_i2s->send_misc_msg_flag = 1;
		else
			pr_err("%s: restore DVFS failed\n", __func__);
		...
		struct snd_soc_dai *dai)
{
	...
	if (atomic_read(&agcp_i2s->open_cnt) == 1) {
		...
		ret = aud_send_cmd_no_param(AMSG_CH_MISC,
			RESTORE_AUD_DVFS, 0, 0, 0, 0, -1);
		if (ret < 0)
			pr_err("%s: restore DVFS failed\n", __func__);

		if (agcp_i2s->send_misc_msg_flag == 1) {
			ret = aud_send_cmd_no_param(AMSG_CH_MISC,
				DISABLE_AUD_PLL_BIND, 0, 0, 0, 0, -1);
			if (ret < 0)
				pr_err("%s: restore DVFS failed\n", __func__);
			agcp_i2s->send_misc_msg_flag = 0;
		}
		pm_runtime_mark_last_busy(dai_data->dev);
		pm_runtime_put_autosuspend(dai_data->dev);
	}
